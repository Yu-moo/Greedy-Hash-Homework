...
def read_image_paths(filename):
    ...
    full_path=[f'/root/autodl-tmp/NUS-WIDE/{path}' for path in paths]
    return full_path,np.array(labels_list)

train_paths,train_L = read_image_paths('nuswide21/train.txt')
test_paths,test_L = read_image_paths('nuswide21/test.txt')
database_paths,database_L = read_image_paths('nuswide21/database.txt')
...
torch.save(cnn.state_dict(), 'nuswide21.pkl')
